...
public class DirectInputDevice : InputDevice, IInputUpdateCallbackReceiver{
	...
	public static void GenerateISLayouts(){
		InputSystem.RegisterLayout<DirectInputDevice>("DirectInput",
			matches: new InputDeviceMatcher().WithInterface("Unity-DirectInput")
		);
	}
	...
	public static bool AddDIDeviceToIS(DeviceInfo device){
		// Check if the device already exists as a DirectInputDevice
		if (InputSystem.devices.OfType<DirectInputDevice>().Any(d => d.description.serial == device.guidInstance)) 
		{
				return true;  // Device already exists, no need to add again
		}
		// Filter out known controllers (like XInput and PlayStation controllers)
		if (IsKnownGameController(device))
		{
				Debug.Log($"Skipping known game controller: {device.productName}");
				return false;  // Skip known game controllers, do not add as DirectInput
		}
		tryToRemoveOriginalDevice(device);
		...
		if (DIManager.Attach(device)){
			...
			InputDevice ISDevice = InputSystem.AddDevice(new InputDeviceDescription {
				...
				product       = $"{device.productName}_DI",
	...
	private static bool IsKnownGameController(DeviceInfo device)
	{
			string lowerProductName = device.productName.ToLower();
			if (lowerProductName.Contains("xinput") || lowerProductName.Contains("xbox"))
			{
					return true;
			}
			if (lowerProductName.Contains("dualshock") || lowerProductName.Contains("playstation"))
			{
					return true;
			}
			return false;
	...
	public static void tryToRemoveOriginalDevice(DeviceInfo device)
	{
		foreach (var dev in InputSystem.devices)
		{
			var distance = CalculateSimilarity(dev.description.product, device.productName);
			Debug.Log($"Score:{distance}, {dev.description.product} vs {device.productName}");
		}
		
		
		var originalDevice = InputSystem.devices.FirstOrDefault(d =>
			d.description.interfaceName != "Unity-DirectInput" &&
			CalculateSimilarity(d.description.product, device.productName) >= 0.50
		);
		if (originalDevice != null)
		{
			Debug.Log($"Removing original device: {originalDevice.description.product}, Serial: {originalDevice.deviceId}");
			InputSystem.RemoveDevice(originalDevice);
		}
	}
	...
	public static double CalculateSimilarity(string s1, string s2)
	{
		...
		return 1.0 - (double)LevenshteinDistance(s1, s2) / maxLen; // Using Levenshtein as an example
	}

	// Example implementation of Levenshtein distance
